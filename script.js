...
const displayProcessedPdfPreview = (pdfBytes) => {
...
// 初期表示時に画像プレビュー領域にメッセージを表示
document.addEventListener('DOMContentLoaded', () => {
    displayImagePreviews([]);
    updateRealtimePreview(); // 初期表示時にリアルタイムプレビューも更新
});

// Function to display image previews
const displayImagePreviews = (imageFileObjects) => {
...
const appendImageToPreview = (src, fileName, index) => {
...
embedButton.addEventListener('click', async () => {
    ...
    try {
        ...
        const firstPage = pages[0]; // 最初のページに画像を配置
...
const updateRealtimePreview = () => {
    ...
    realtimePreviewContainer.innerHTML = ''; // 以前のプレビューをクリア

    if (selectedImageFiles.length === 0) {
        realtimePreviewContainer.innerHTML = '<p>画像ファイルを選択するとここにプレビューが表示されます。</p>';
        console.log('updateRealtimePreview: 画像ファイルが選択されていません。メッセージを表示しました。');
        return;
    }
